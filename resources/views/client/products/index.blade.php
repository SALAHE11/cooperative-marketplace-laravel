                        ...
                        <div class="col-xl-4 col-lg-6 col-md-6 mb-4">
                            <div class="card product-card">
                                <div class="position-relative">
                                    ...
                                    @endif
                                    <div class="price-badge">{{ number_format($product->price, 2) }} MAD</div>
                                </div>
...
function fetchSearchSuggestions(query) {
    fetch(`{{ route('client.products.search') }}?q=${encodeURIComponent(query)}`)
                ...
                suggestionsContainer.innerHTML = data.products.map(product => `
                    <div class="search-suggestion" onclick="selectProduct('${product.name}')">
                        <div class="d-flex align-items-center">
                            ${product.image ? `<img src="${product.image}" style="width: 40px; height: 40px; object-fit: cover;" class="me-2">` : '<div class="me-2" style="width: 40px; height: 40px; background: #f8f9fa; display: flex; align-items: center; justify-content: center;"><i class="fas fa-image text-muted"></i></div>'}
                            <div>
...
function addToCart(productId) {
    fetch(`{{ route('client.cart.add', '') }}/${productId}`, {
        method: 'POST',
        headers: {
